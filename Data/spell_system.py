...
def handle_damage_spell(caster, target, spell, dungeon):
    ...
    if effects:
        ...
        # The 'return messages' line has been removed from here.
...
def handle_utility_spell(caster, target, spell, dungeon):
    ...
    #--------------------------------------------------------------------------
    # Apply generic conditions if defined in the spell
    # This block will execute if none of the above elif conditions for specific utility spells were met,
    # OR if a specific utility spell was handled but also has generic effects listed.
    # We should ensure conditions are not doubly applied if a specific handler already did.
    # For now, we assume ConditionManager handles this or it's an edge case.

    effects = get_spell_effects(spell)
    if effects:
        from condition_system import condition_manager, ConditionType, Condition # Make sure Condition is imported

        logger.debug(f"Applying generic effects for utility spell {spell_name}: {effects} with duration {duration}")

        for effect_name in effects:
            # Skip applying if this specific effect was already handled by a utility_type block
            # Example: Invisibility spell's "Invisible" effect is handled by utility_type == "invisibility"
            if utility_type == "invisibility" and effect_name.upper() == "INVISIBLE":
                logger.debug(f"Skipping generic application of {effect_name} for {spell_name} as it was handled by specific utility logic.")
                continue
            
            condition_type_enum = None
            try:
                condition_type_enum = ConditionType[effect_name.upper()]
            except KeyError:
                logger.warning(f"Unknown condition effect_name: {effect_name} in utility spell {spell_name}")
                continue

            if condition_type_enum:
                severity = spell.get("severity", 1)
                condition = Condition(condition_type_enum, duration, caster, severity)
                result_message = condition_manager.apply_condition(target, condition)
                if result_message not in messages: # Avoid duplicate messages if somehow already added
                    messages.append(result_message)
                logger.info(f"Applied generic condition {effect_name} to {target.name} from utility spell {spell_name}")

    # If no messages were generated by specific utility handlers or generic effect application,
    # then add the default casting message.
    if not messages: 
